...
describe('MongoFS', function() {
	...
	describe('.get(path, callback(err, file))', function() {
		before(function(done) {
			coll.insert({
				...
				f : {
					a: [{foo: 'bar', _ts:1}],
					b: [{value:'first', _ts:'0200'}, {value:'last', _ts:'0100'}, ]
				}
			}, done);
		});
	...
	describe('.remove(path, timestamp, callback(err))', function(){
		beforeEach(function(done) {
			mfs.put('/file/to/delete', {foo: 'bar', _ts: '01000'}, done);
		});
	...
	describe('.transaction(trans, callback(err, actions))', function() {
		...
		describe('get', function() {
			before(function(done) {
				util.seq([
					function(_) { mfs.transaction({_ts: '01000', path:'/bank/account/', put: {me: {amount: 100}}}, _); },
					function(_) { mfs.transaction({_ts: '02000', path:'/bank/account/', put: {me: {amount: 200}}}, _); },
					function(_) { mfs.transaction({_ts: '03000', path:'/bank/account/', put: {me: {amount: 300}}}, _); },
				], done)();
			});
